...
export default function Effects({ synth }) {
  ...
  const [shifter, setShifter] = useState(0.5);
  ...
  const [delayValue, setDelayValue] = useState(0.5);
  ...
  useEffect(() => {
    mydelay = new Tone.PingPongDelay(0.5, 0.6).toDestination();
    reverb = new Tone.Reverb(5).toDestination();
    shift = new Tone.FrequencyShifter(0.5).toDestination();
    vibrato = new Tone.Vibrato(6, 0.1).toDestination();
